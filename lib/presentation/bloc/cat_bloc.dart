...
class CatBloc extends Bloc<CatEvent, CatState> {
  ...
  Future<void> _onSearchBreeds(SearchBreeds event, Emitter<CatState> emit) async {
    ...
    result.fold(
      (failure) => emit(state.copyWith(error: failure.message ?? 'Search failed', isLoading: false)),
      (breedsResult) async {
        ...
        imagesResult.fold(
          (failure) => emit(state.copyWith(error: failure.message ?? 'Load images failed', isLoading: false)),
          (images) {
            emit(state.copyWith(images: images, isLoading: false, hasReachedMax: images.length < 20, breedIds: breedIds));
          },
        );
      },
    );
  ...
  Future<void> _onLoadMoreImages(LoadMoreImages event, Emitter<CatState> emit) async {
    ...
    imagesResult.fold(
      (failure) => emit(state.copyWith(error: failure.message ?? 'Load more failed', isLoading: false)),
      (images) {
        if (images.isEmpty) {
  ...
  Future<void> _onGetFact(GetFact event, Emitter<CatState> emit) async {
    ...
    factResult.fold(
      (failure) => emit(state.copyWith(error: failure.message ?? 'Get fact failed', isLoading: false)),
      (fact) {
  ...
  Future<void> _onRefreshImages(RefreshImages event, Emitter<CatState> emit) async {
    ...
    final imagesResult = await getImages.call(page: 0);
    imagesResult.fold(
      (failure) => emit(state.copyWith(error: failure.message ?? 'Refresh failed', isLoading: false)),
      (images) {
        emit(state.copyWith(images: images, isLoading: false, hasReachedMax: images.length < 20));
      },
    );
